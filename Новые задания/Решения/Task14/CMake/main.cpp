...
void filter_image(std::ofstream& output_file, const std::vector<char>& pixels) {
    ...
    std::vector<int> ps(new_row_size * new_height + new_width * colorsChannels);
    ...
    std::vector<std::vector<int>> sums_pixels(width);
...
void medianFilter(const std::string INPUT_PATH, const std::string OUTPUT_PATH) {
    ...
    std::vector<char> special_pixels = create_pixels_vector(pixels);
