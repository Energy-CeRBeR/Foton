...
void rotate_nn(const std::string INPUT_PATH, const std::string OUTPUT_PATH, int angle) {
    ...
    int outputWidth = abs(fileInfoHeader.biWidth * abs(cos(radian)) + fileInfoHeader.biHeight * abs(sin(radian)));
    int outputHeight = abs(fileInfoHeader.biWidth * abs(sin(radian)) + fileInfoHeader.biHeight * abs(cos(radian)));
    ...
    std::cout << width << " " << outputWidth << " " << outputWidth - width << std::endl;
    std::cout << height << " " << outputHeight << " " << outputHeight - height << std::endl;
    for (int i = 0; i < outputHeight; ++i) {
        for (int j = 0; j < outputWidth; ++j) {
            double x = (j - cx) * cos(radian) - (i - cy) * sin(radian) + cx - (outputWidth - width) / 2;
            double y = (j - cx) * sin(radian) + (i - cy) * cos(radian) + cy - (outputHeight - height) / 2;
...
void rotate_bln(const std::string INPUT_PATH, const std::string OUTPUT_PATH, int angle) {
    ...
    for (int i = 0; i < outputHeight; ++i) {
        for (int j = 0; j < outputWidth; ++j) {
            double x = (j - cx) * std::cos(radian) - (i - cy) * std::sin(radian) + cx - (outputWidth - width) / 2;;
            double y = (j - cx) * std::sin(radian) + (i - cy) * std::cos(radian) + cy - (outputHeight - height) / 2;
